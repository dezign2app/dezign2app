...
export const useChatStore = create<ChatStore>()(
  persist(
    (set) => ({
      ...
      streamingThinking: "",
      setStreamingThinking: (text: string) =>
        set((state) => {
      ...
      selectedContext: [],
      showAIPopup: false,
      setShowAIPopup: (show: boolean | typeof TOGGLE_POPUP) =>
        set((state) => ({
          showAIPopup: show === TOGGLE_POPUP ? !state.showAIPopup : show,
        })),
      setSelectedContext: (selectedText: SelectedContext | "reset") =>
        set((state) => {
          if (selectedText === "reset") return { selectedContext: [] };
          const isOverlapping = state.selectedContext.some((existing) => {
            return (
              existing.from === selectedText.from &&
              existing.to === selectedText.to
            );
          });
