...
export const VectorDBRefNode = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "px-3 py-2 border-b flex items-center justify-between group rounded-t-xl bg-violet-500/10 text-violet-700 dark:text-violet-400",
        )}
      >
        ...
        <div className="px-3 pb-2 flex flex-col gap-1.5">
          {accessors.length === 0 ? (
            ...
            accessors.map((acc) => (
              <div
                key={acc.id}
                className="flex flex-col px-2 py-1 border-l-2 border-violet-500/60 bg-background/50 rounded-r gap-0.5"
              >
