...
export const SearchIndexConfig = ({
  ...
  nodeId,
  sourceId,
}: SearchIndexConfigProps) => {
  const nodes = useBackendCanvasStore((s) => s.nodes);
  const updateNode = useBackendCanvasStore((s) => s.updateNode);
  const parentNode = nodes.find((n) => n.id === nodeId);
  if (!parentNode) return null;
  const source: SearchSource | undefined = parentNode.data.searchSources?.find(
    (candidate) => candidate.id === sourceId,
  );
  const item: SearchIndexItem | undefined = source?.indexes.find(
    (index) => index.id === id,
  );
  if (!source || !item) return null;
  const handleUpdate = (changes: Partial<SearchIndexItem>) =>
    updateNode(nodeId, {
      data: {
        ...parentNode.data,
        searchSources: parentNode.data.searchSources?.map((candidate) =>
          candidate.id === sourceId
            ? {
                ...candidate,
                indexes: candidate.indexes.map((index) =>
  ...
  const tableRef = nodes.find(
    (node) => node.id === source.dbTable && node.type === "db_ref",
  );
  const table = nodes.find(
    (node) => node.id === tableRef?.data.tableRef && node.type === "entity",
  );
  const tableName =
    tableRef?.data.label || table?.data.label || "the selected table";
    ...
    <div className="flex flex-col gap-6 mt-6 pb-12">
      <div className="flex flex-col gap-2 border-b border-border/50 pb-6">
        ...
        <span className="text-sm text-muted-foreground">
          This index is derived from {tableName}. Table and ingestion settings
          are managed on the source.
        </span>
      </div>
