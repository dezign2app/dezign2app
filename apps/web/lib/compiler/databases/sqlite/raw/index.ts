...
function generateTableHelpers(
  ...
  dbOps.forEach((op) => {
    ...
    if (effectiveCode && effectiveCode.trim()) {
      ...
      code += `export function ${op.name}(${pkVarName}: ${pkTs}): ${Pascal}Row | undefined {\n`;
      ...
      code += `  const ${pkVarName} = typeof info.lastInsertRowid === "bigint" ? info.lastInsertRowid.toString() : String(info.lastInsertRowid);\n`;
      code += `  return { ${pkVarName}, ...data } as ${Pascal}Row;\n`;
      code += `}\n\n`;
    } else if (op.kind === "update" && writableCols.length > 0) {
      code += `/** ${op.description || `Update a ${tableName} row by primary key`} */\n`;
      code += `export function ${op.name}(${pkVarName}: ${pkTs}, data: Update${Pascal}Data): ${Pascal}Row | undefined {\n`;
      code += `  const current = find${Pascal}ById(${pkVarName});\n`;
      ...
      code += `  return find${Pascal}ById(${pkVarName});\n`;
      code += `}\n\n`;
    } else if (op.kind === "delete") {
      code += `/** ${op.description || `Delete a ${tableName} row by primary key`} */\n`;
      code += `export function ${op.name}(${pkVarName}: ${pkTs}): void {\n`;
