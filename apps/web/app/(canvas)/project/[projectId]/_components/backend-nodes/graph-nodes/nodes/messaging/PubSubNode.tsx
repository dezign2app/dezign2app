...
export const PubSubNode = ({ id, data, selected }: NodeProps<BackendNode>) => {
  ...
  const publishers = publisherEdges
    .map((edge) => {
  ...
  const subscribers = subscriberEdges
    .map((edge) => {
        ...
        <div className="px-3 pb-2 flex flex-col gap-1">
          {publishers.length === 0 ? (
            <span className="text-[10px] text-muted-foreground italic px-1">
              No publishers connected
            </span>
          ) : (
            publishers.map((p) => (
              <div
                key={p.id}
                className="text-xs font-medium truncate px-1 border-l-2 border-purple-500/50 ml-1"
              >
                {p.label}
              </div>
            ))
            ...
            <div className="px-3 py-2 flex flex-col gap-3 border-t text-[10px] text-muted-foreground bg-secondary/5">
                ...
                <>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, gcpTopic: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      ...
                      value={data.gcpSubscription || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, gcpSubscription: e.target.value },
                        })
                      }
                    />
                  </div>
              ...
              {implementation === "Redis Pub/Sub" && (
                <p className="text-[10px] text-muted-foreground italic">
                  Redis Pub/Sub is fire-and-forget — no persistence or
                  acknowledgment. No additional config required.
                </p>
              )}
            </div>
          )}
