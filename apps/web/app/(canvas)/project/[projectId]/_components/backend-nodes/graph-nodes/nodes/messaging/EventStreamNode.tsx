...
const EVENTSTREAM_IMPLEMENTATIONS = [
  "None",
  "Kafka",
  "Redis Streams",
  "Azure Event Hubs",
        ...
        <div className="px-3 pb-2 flex flex-col gap-1">
          {producers.length === 0 ? (
            ...
            producers.map((p) => (
              <div
                key={p.id}
                className="text-xs font-medium truncate px-1 border-l-2 border-teal-500/50 ml-1"
              >
                {p.label}
              </div>
            ))
            ...
            <div className="px-3 py-2 flex flex-col gap-3 border-t text-[10px] text-muted-foreground bg-secondary/5">
                ...
                <>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, kafkaPartitions: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      className="h-6 text-[10px] w-16 text-right bg-background nodrag"
                      placeholder="e.g. 3"
                      value={data.kafkaReplication || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, kafkaReplication: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Select
                      value={data.kafkaCompression || "None"}
                      onValueChange={(val) =>
                        updateNode(id, {
                          data: { ...data, kafkaCompression: val },
                        })
                      }
                    >
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, kafkaTTL: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      value={data.kafkaBatchSize || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, kafkaBatchSize: e.target.value },
                        })
                      }
                    />
                  </div>
              ...
              {implementation === "Redis Streams" && (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Input
                    className="h-6 text-[10px] w-32 bg-background nodrag"
                    placeholder="Group Name"
                    value={data.redisConsumerGroup || ""}
                    onChange={(e) =>
                      updateNode(id, {
                        data: { ...data, redisConsumerGroup: e.target.value },
                      })
                    }
                  />
                </div>
              ...
              {implementation === "Azure Event Hubs" && (
                <p className="text-[10px] text-muted-foreground italic">
                  Azure Event Hubs uses the common delivery and retention
                  settings above. No additional config required.
                </p>
              )}
            </div>
          )}
