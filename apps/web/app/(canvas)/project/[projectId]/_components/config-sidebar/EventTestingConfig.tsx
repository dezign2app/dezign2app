...
export const EventTestingConfig = ({
  id,
  nodeId,
  targetNodeId,
  endpointId,
  initialTab = "trigger",
}: EventTestingConfigProps) => {
  ...
  const event = parentNode?.data?.events?.find(
    (e: UIEventItem) => e.id === id,
  ) as UIEventItem | undefined;
  ...
  let endpoint =
    ...
    targetNode?.data?.endpoints?.find((e: Endpoint) => e.id === endpointId) ||
    (
      targetNode?.data?.routeGroups?.flatMap(
        (g: any) => g.endpoints || [],
      ) as Endpoint[]
  ...
  if (!endpoint && targetNode) {
    if (messagingTypes.includes(targetNode.type)) {
      const resourceList =
        ...
        [];
      const resource =
        resourceList.find((r: any) => r.id === endpointId) || resourceList[0];
      ...
      for (const edge of outgoingEdges) {
        ...
        if (isMessaging) {
          ...
          const resourceList =
            ...
            [];
          const resource =
            resourceList.find((r: any) => r.id === resourceId) ||
            resourceList[0];
  ...
  const handleCreateNew = (caseName: string) => {
    ...
    const newCase: SimulationTestCase = {
      ...
      expectedBody:
        endpoint.simulationOutput !== undefined
          ? (endpoint.simulationOutput as JSONValue)
          : endpoint.responseBody?.rawJson
            ? (() => {
                try {
                  return JSON.parse(endpoint.responseBody.rawJson) as JSONValue;
                } catch {
                  return undefined;
                }
              })()
    ...
    <div className="flex flex-col gap-6 mt-6 pb-12 font-sans">
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          <TabsContent
            value="test-cases"
            className="flex flex-col gap-4 p-1 m-0"
          >
