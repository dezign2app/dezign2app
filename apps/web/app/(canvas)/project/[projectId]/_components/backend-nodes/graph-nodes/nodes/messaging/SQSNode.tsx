...
export const SQSNode = ({ id, data, selected }: NodeProps<BackendNode>) => {
  ...
  const updateBroker = <
    K extends keyof NonNullable<BackendNode["data"]["sqsBroker"]>,
  >(
    key: K,
    value: NonNullable<BackendNode["data"]["sqsBroker"]>[K],
  ) => {
    updateNode(id, {
      data: {
        ...data,
          ...
          <div className="px-3 py-2 flex flex-col gap-3 border-t text-[10px] text-muted-foreground bg-secondary/5">
            <div className="flex items-center justify-between gap-2">
              ...
              <Input
                ...
                placeholder="e.g. 30s"
                value={broker.visibilityTimeout || ""}
                onChange={(e) =>
                  updateBroker("visibilityTimeout", e.target.value)
                }
              />
            </div>
            <div className="flex items-center justify-between gap-2">
              ...
              <Input
                ...
                placeholder="e.g. 0s"
                value={broker.delay || ""}
                onChange={(e) => updateBroker("delay", e.target.value)}
              />
            </div>
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                checked={broker.fifo || false}
                onCheckedChange={(val) => updateBroker("fifo", val)}
              />
            </div>
          </div>
        )}
