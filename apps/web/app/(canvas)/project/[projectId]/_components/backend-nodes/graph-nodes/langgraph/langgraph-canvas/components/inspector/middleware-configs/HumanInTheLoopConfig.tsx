    ...
    <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
      ...
      <div className="flex flex-col gap-2">
        <Label className="text-xs font-semibold text-foreground">
          Interrupt On Actions
        </Label>
        <div className="flex flex-col gap-2 p-2 bg-background border border-border/50 rounded-lg">
          {[
            {
              key: "writeFile",
              label: "File System Modifications (writeFile)",
            },
            {
              key: "executeCode",
              label: "Arbitrary Code Execution (executeCode)",
            },
            {
              key: "deleteData",
              label: "Database Deletions / Mutating Queries",
            },
            { key: "apiCall", label: "External API Endpoint Calls" },
          ].map(({ key, label }) => {
            const checked =
              data.humanInTheLoopConfig?.interruptOn?.[key] ??
              key === "writeFile";
            return (
              <div
                key={key}
