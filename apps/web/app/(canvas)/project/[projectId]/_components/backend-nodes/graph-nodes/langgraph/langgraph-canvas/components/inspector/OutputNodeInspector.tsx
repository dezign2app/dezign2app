...
import { Radio, Trash2, Zap, Plug, Globe, Sparkles, Layers, Info, Cpu, Link2 } from "lucide-react";
...
function isStreamContentMode(val: string): val is "ai_node_tokens" | "structured_output" | "step_output" | "full_state" {
  return val === "ai_node_tokens" || val === "structured_output" || val === "step_output" || val === "full_state";
}
...
export function OutputNodeInspector({
  ...
  const toggleRouteBinding = (edgeId: string) => {
    if (boundRouteIds.includes(edgeId)) {
      onUpdateOutput({ boundRouteIds: boundRouteIds.filter((id) => id !== edgeId) });
    } else {
      ...
      <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="ai_node_tokens" className="text-xs">🤖 AI Node Tokens (Real-time LLM streamMode)</SelectItem>
              <SelectItem value="structured_output" className="text-xs">📦 Structured Output Stream (Zod / JSON Schema)</SelectItem>
              <SelectItem value="step_output" className="text-xs">⚡ Target Step Output Payload</SelectItem>
              <SelectItem value="full_state" className="text-xs">📊 Full Graph State Object</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {contentMode === "ai_node_tokens" && availableLLMNodes.length > 0 && (
          <div className="flex flex-col gap-1.5">
            <Label className="text-xs font-semibold">Source AI / LLM Node</Label>
            <Select
              value={selectedOutputData.sourceStepId || "__first__"}
              onValueChange={(val) => onUpdateOutput({ sourceStepId: val === "__first__" ? undefined : val })}
            >
              <SelectTrigger className="bg-background text-xs h-8 font-mono">
                <SelectValue placeholder="First LLM Node (Default)" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__first__" className="text-xs font-mono">Default / First AI Node</SelectItem>
                {availableLLMNodes.map((llm) => (
                  <SelectItem key={llm.id} value={llm.id} className="text-xs font-mono">
        ...
        {connectedRoutes.length > 0 && (
          <div className="flex flex-col gap-2 pt-2 border-t border-border/40">
            ...
            <p className="text-[10px] text-muted-foreground">
              Select which entry points emit over this output channel (leave empty for all routes):
            </p>
