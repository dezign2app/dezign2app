...
export const RedisStreamsNode = ({
  ...
  const updateBroker = <
    K extends keyof NonNullable<BackendNode["data"]["redisBroker"]>,
  >(
    key: K,
    value: NonNullable<BackendNode["data"]["redisBroker"]>[K],
  ) => {
    updateNode(id, {
      data: {
        ...data,
          ...
          <div className="px-3 py-2 flex flex-col gap-3 border-t text-[10px] text-muted-foreground bg-secondary/5">
            <div className="flex items-center justify-between gap-2">
              ...
              <Input
                ...
                value={broker.consumerGroup || ""}
                onChange={(e) => updateBroker("consumerGroup", e.target.value)}
              />
            </div>
          </div>
        )}
