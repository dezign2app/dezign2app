...
export const TestCaseEditor = ({
  ...
  const [headers, setHeaders] = useState<Parameter[]>(
    () =>
      endpoint.headers?.map((h) => ({
        ...
        key: h.key ?? h.name,
        value:
          initialCase.request?.headers?.[(h.name || "").toLowerCase()] ??
          h.defaultValue ??
          "",
      })) || [],
  );
  ...
  const [params, setParams] = useState<Parameter[]>(() =>
    endpointInputParams(endpoint).map((param) => ({
      ...param,
      value:
        initialCase.request?.params?.[param.key || param.name] ??
        param.value ??
        "",
    })),
  );
        ...
        <div className="flex flex-col gap-3">
          {params.map((param, idx) => (
            <div key={`param-${idx}`} className="flex items-center gap-2">
              <Label className="text-xs font-mono text-muted-foreground w-24 truncate">
          ...
          return (
            <div key={m.id} className="flex flex-col gap-2 pt-3 border-t">
              <h5 className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1.5">
                <span className="normal-case font-mono bg-background border px-1 rounded text-[9px]">
                  {m.label}
                </span>
                <span
                  className={`px-1 py-0.5 rounded text-[8px] uppercase tracking-wider font-mono border ${badgeColor}`}
                >
