...
import { Wrench, Trash2, Box, Server, Globe, BoxSelect, AlertCircle, Database, Check, Network, ShieldAlert } from "lucide-react";
...
export function ToolNodeInspector({
  ...
  const [schemaText, setSchemaText] = useState(selectedToolData.inputSchema || "");
  ...
  const isContentBlocksReturn = selectedToolData.returnType === "content_blocks";
  
  const handleNameChange = (val: string) => {
    const snake = val.trim().toLowerCase().replace(/[^a-z0-9_]/g, "_");
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Core Config</h3>
        </div>

        <div className="flex flex-col gap-2">
          <Label className="text-xs font-semibold text-foreground">Name (snake_case)</Label>
          <LocalInput
          ...
          >
            ...
            <SelectContent>
              <SelectItem value={TOOL_SOURCE_INLINE}>
                <div className="flex items-center gap-2"><Box className="w-3.5 h-3.5 text-emerald-400" /> Inline Code</div>
              </SelectItem>
              <SelectItem value={TOOL_SOURCE_MCP_SERVER}>
                <div className="flex items-center gap-2"><Server className="w-3.5 h-3.5 text-orange-400" /> MCP Server</div>
              </SelectItem>
              <SelectItem value={TOOL_SOURCE_API_ENDPOINT}>
                <div className="flex items-center gap-2"><Globe className="w-3.5 h-3.5 text-blue-400" /> API Endpoint</div>
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {selectedToolData.source === TOOL_SOURCE_API_ENDPOINT && (
          <div className="flex flex-col gap-2 p-2 bg-blue-500/5 rounded-lg border border-blue-500/10">
            <Label className="text-[10px] font-semibold text-blue-500 uppercase tracking-wider">Endpoint URL</Label>
            <LocalInput
        ...
        {selectedToolData.source === TOOL_SOURCE_MCP_SERVER && (
          <div className="flex flex-col gap-2 p-2 bg-orange-500/5 rounded-lg border border-orange-500/10">
            <Label className="text-[10px] font-semibold text-orange-500 uppercase tracking-wider">MCP Connection ID</Label>
            <LocalInput
              value={selectedToolData.mcpConnectionId || ""}
              onChange={(e) => onUpdateTool({ mcpConnectionId: e.target.value })}
              className="h-7 text-xs font-mono bg-background"
              placeholder="conn_12345"
            />
            <Label className="text-[10px] font-semibold text-orange-500 uppercase tracking-wider mt-2">Remote Tool Name</Label>
            <LocalInput
        ...
        <div className="flex flex-col gap-2 mt-2">
          <Label className="text-xs font-semibold text-foreground flex justify-between items-center">
            ...
            <span className="text-[9px] font-normal text-muted-foreground">Zod compatible</span>
          </Label>
          <LocalTextarea
            value={schemaText}
            onChange={(e) => {
      ...
      <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Return Behavior</h3>
          </div>
          <div className="flex items-center gap-2" title="Return output directly, skipping further model processing">
            <Label htmlFor="returnDirect" className="text-xs font-semibold cursor-pointer">Return Direct</Label>
            <Switch
              id="returnDirect"
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="string"><span className="font-mono text-xs text-foreground">string</span></SelectItem>
              <SelectItem value="object"><span className="font-mono text-xs text-amber-500">object (dict)</span></SelectItem>
              <SelectItem value="content_blocks"><span className="font-mono text-xs text-sky-500">content_blocks (multimodal)</span></SelectItem>
              <SelectItem value="command"><span className="font-mono text-xs text-purple-500">command (state updates)</span></SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {isContentBlocksReturn && (
          <div className="flex gap-2 p-2 rounded bg-sky-500/10 border border-sky-500/20 items-start">
            ...
            <p className="text-[10px] text-sky-500 leading-tight">
              Multimodal returns (images/audio) require a model that supports them.
              Check the step's model configuration.
            </p>
          </div>
        )}
        ...
        {isObjectReturn && (
          <div className="flex flex-col gap-2 mt-2">
            <Label className="text-xs font-semibold text-foreground">Output Schema (JSON Schema)</Label>
            <LocalTextarea
        ...
        {isCommandReturn && (
          <div className="flex flex-col gap-2 mt-2 bg-purple-500/5 p-2 rounded border border-purple-500/10">
            <div className="flex items-center justify-between">
              <Label className="text-[10px] font-semibold text-purple-500 uppercase">State Updates</Label>
            </div>
            {(selectedToolData.commandConfig?.stateUpdates || []).map((update, idx) => (
              <div key={idx} className="flex flex-col gap-1.5 p-2 bg-background border border-border/50 rounded text-xs">
                <div className="flex items-center gap-2">
                  <Select
                  ...
                  >
                    <SelectTrigger className="h-6 text-[10px] flex-1 font-mono"><SelectValue placeholder="Channel" /></SelectTrigger>
                    <SelectContent>
                      {stateChannels.map(c => (
                        <SelectItem key={c.key} value={c.key} className="text-[10px] font-mono">{c.key}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="set" className="text-[10px]">set</SelectItem>
                      <SelectItem value="append" className="text-[10px]">append</SelectItem>
                      <SelectItem value="expression" className="text-[10px] text-amber-500">expression</SelectItem>
                    </SelectContent>
                  </Select>
                  <button
                    type="button"
                ...
                {update.mode === "expression" && (
                  <div className="flex flex-col gap-1 mt-1">
                    ...
                    <span className="text-[9px] text-amber-500 leading-tight">
                      Expressions run in a safe DSL. Available vars: `priorState`, `toolResult`, `input`.
                    </span>
                  </div>
                )}
              </div>
            ...
            <button
              ...
              onClick={() => {
                const newUpdates = [...(selectedToolData.commandConfig?.stateUpdates || []), { channelKey: stateChannels[0]?.key || "messages", mode: "set" as const }];
                handleUpdateCommandConfig({ stateUpdates: newUpdates });
              }}
            >
              + Add State Update
      ...
      {/* ─── 3. Headless Mode ──────────────────────────────────────────────── */}
      <div className={`flex flex-col gap-3 p-3 rounded-xl border transition-colors ${isHeadless ? "bg-purple-500/10 border-purple-500/30" : "bg-secondary/10 border-border/50"}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Globe className={`w-4 h-4 ${isHeadless ? "text-purple-500" : "text-muted-foreground"}`} />
            <h3 className={`text-xs font-bold uppercase tracking-wider ${isHeadless ? "text-purple-500" : "text-muted-foreground"}`}>Headless Tool</h3>
          </div>
          <Switch
            checked={isHeadless}
        ...
        {isHeadless && (
          <div className="flex items-start gap-2 text-[10px] text-purple-500/80 leading-tight bg-purple-500/5 p-2 rounded">
            ...
            <p>
              Headless tools are defined on the server but implemented entirely on the client. 
              <strong> Function Body and Server Context settings are disabled.</strong>
              <br/><br/>
              Use <code>.implement(async (args) =&gt; ...)</code> on the client to bind logic.
            </p>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Advanced Server Config</h3>
            </div>

            <div className="flex flex-col gap-3 p-2 bg-background/50 rounded-lg border border-border/50">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-foreground">Context Access</Label>
                <Switch
                  checked={selectedToolData.contextAccess?.enabled || false}
                  onCheckedChange={(c) => onUpdateTool({ contextAccess: { ...selectedToolData.contextAccess, enabled: c } })}
                  className="scale-75 origin-right"
                />
              </div>
              {selectedToolData.contextAccess?.enabled && (
                <div className="flex flex-col gap-1.5 mt-1">
                  <Label className="text-[10px] text-muted-foreground">Whitelisted Fields (comma-separated)</Label>
                  <LocalInput
                    value={selectedToolData.contextAccess?.fields?.join(", ") || ""}
                    onChange={(e) => {
                      const fields = e.target.value.split(",").map(s => s.trim()).filter(Boolean);
                      onUpdateTool({ contextAccess: { ...selectedToolData.contextAccess, fields, enabled: true } });
                    }}
                    className="h-6 text-[10px] font-mono"
                    placeholder="userId, tenantId"
                  />
                  <span className="text-[9px] text-muted-foreground">Fields will be passed via scoped `config.context`.</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 p-2 bg-background/50 rounded-lg border border-border/50">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-foreground">Store Access</Label>
                <Switch
                  checked={selectedToolData.storeAccess?.enabled || false}
                  onCheckedChange={(c) => onUpdateTool({ storeAccess: { ...selectedToolData.storeAccess, enabled: c } })}
                  className="scale-75 origin-right"
                />
              </div>
              {selectedToolData.storeAccess?.enabled && (
                <div className="flex flex-col gap-3 mt-1">
                  <div className="flex flex-col gap-1.5">
                    <Label className="text-[10px] text-muted-foreground">Store Namespace</Label>
                    <LocalInput
                      value={selectedToolData.storeAccess?.namespace || ""}
                      onChange={(e) => onUpdateTool({ storeAccess: { ...selectedToolData.storeAccess, namespace: e.target.value, enabled: true } })}
                      className="h-6 text-[10px] font-mono"
                      placeholder="users"
                    />
                  </div>
                      ...
                      {(["get", "put", "delete", "list"] as const).map((op) => {
                        ...
                        return (
                          <div key={op} className="flex items-center gap-1.5">
                            ...
                            />
                            <Label htmlFor={`store-op-${op}`} className="text-[10px] font-mono cursor-pointer">{op}</Label>
                          </div>
                        );
                      })}
                    </div>
                  </div>
                  <span className="text-[9px] text-muted-foreground">Injected as scoped store wrapper avoiding global access.</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between p-2 bg-background/50 rounded-lg border border-border/50">
              <div className="flex flex-col gap-0.5">
                <Label className="text-xs font-semibold text-foreground">Stream Writer</Label>
                <span className="text-[9px] text-muted-foreground">Enable real-time custom chunk streaming</span>
              </div>
              <Switch
      ...
      <div className="flex flex-col gap-4 p-3 bg-secondary/10 rounded-xl border border-border/50">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Error Handling</h3>
          </div>
          <Switch
            checked={selectedToolData.errorHandling?.enabled || false}
            onCheckedChange={(c) => onUpdateTool({ errorHandling: { ...selectedToolData.errorHandling, enabled: c } })}
            className="scale-75 origin-right"
          />
        </div>
          ...
          <>
            <div className="flex items-center justify-between gap-2">
              ...
              <LocalInput
                ...
                onChange={(e) => onUpdateTool({ errorHandling: { ...selectedToolData.errorHandling, retryCount: parseInt(e.target.value) || 0, enabled: true } })}
              />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <LocalTextarea
                ...
                onChange={(e) => onUpdateTool({ errorHandling: { ...selectedToolData.errorHandling, customErrorMessage: e.target.value, enabled: true } })}
              />
              <span className="text-[9px] text-muted-foreground leading-tight">
                Generates a per-tool scoped middleware to wrap calls and inject this message on failure.
              </span>
            </div>
          </>
        )}
