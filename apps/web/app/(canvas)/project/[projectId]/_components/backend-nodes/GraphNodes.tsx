...
const EndpointRow = ({
  ...
  const deleteHeader = (id: string) => {
    handleUpdateItem(item.id, {
      headers: (item.headers || []).filter((h) => h.id !== id),
    });
  };
  ...
  const updateParam = (id: string, key: string, type: string) => {
    handleUpdateItem(item.id, {
      queryParams: (item.queryParams || []).map((p) =>
        p.id === id ? { ...p, key, type } : p,
      ),
    });
  };
  const deleteParam = (id: string) => {
    handleUpdateItem(item.id, {
      queryParams: (item.queryParams || []).filter((p) => p.id !== id),
    });
  };
              ...
              {(item.queryParams || []).map((p: Parameter) => (
                <div key={p.id} className="flex items-center gap-1">
                  <Input
                    ...
                    placeholder="Name"
                    value={p.key || p.name || ""}
                    onChange={(e) => updateParam(p.id, e.target.value, p.type)}
                  />
                  <Select
                    value={p.type}
                    onValueChange={(v) =>
                      updateParam(p.id, p.key || p.name || "", v)
                    }
                  >
                    <SelectTrigger className="h-6 w-[70px] text-[10px] px-1.5 py-0 nodrag">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
...
const EndpointList = ({
  ...
  const handleUpdate = (id: string, name: string, type: string) => {
    const newItems = items.map((item: Endpoint) =>
      item.id === id ? { ...item, name, type } : item,
    );
    updateNode(nodeId, { data: { ...data, [field]: newItems } });
  };
...
const NodeHeader = ({
  ...
  return (
    <div
      className={cn(
        "px-3 py-2 border-b flex items-center justify-between group rounded-t-xl",
        colorClass,
      )}
    >
...
export const WebClientNode = ({ id, data, selected }: NodeProps<AppNode>) => {
  ...
  return (
    <div
      className={cn(
        "shadow-md rounded-xl bg-card border-2 min-w-[200px] max-w-[300px] flex flex-col",
...
export const ExternalNode = ({ id, data, selected }: NodeProps<AppNode>) => {
  ...
  return (
    <div
      className={cn(
        "shadow-md rounded-xl bg-card border-2 border-dashed min-w-[250px] max-w-[350px] flex flex-col",
        selected ? "border-primary" : "border-border",
      )}
    >
      <NodeHeader
        ...
        icon={Globe}
        title="External API"
        colorClass="bg-gray-500/10 text-gray-700 dark:text-gray-400"
        selected={selected}
      />
