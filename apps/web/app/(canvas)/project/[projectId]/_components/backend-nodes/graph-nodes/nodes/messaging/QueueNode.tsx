      ...
      <div className="p-3 flex flex-col gap-3 bg-secondary/10 border-b">
        <div className="flex items-center justify-between gap-2">
          ...
          <Select
            ...
            onValueChange={(val) =>
              updateNode(id, { data: { ...data, implementation: val } })
            }
          >
            <SelectTrigger className="h-6 w-[160px] text-[10px] px-2 py-0 nodrag">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {QUEUE_IMPLEMENTATIONS.map((impl) => (
                <SelectItem key={impl} value={impl} className="text-xs">
                  {impl}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="px-3 pb-2 flex flex-col gap-1">
          {producers.length === 0 ? (
            ...
            producers.map((p) => (
              <div
                key={p.id}
                className="text-xs font-medium truncate px-1 border-l-2 border-orange-500/50 ml-1"
              >
                {p.label}
              </div>
            ))
          ...
          {showReliability && (
            <div className="p-3 flex flex-col gap-3 bg-secondary/5 border-t">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <Select
                  value={data.failureHandling || "Retry"}
                  onValueChange={(val) =>
                    updateNode(id, { data: { ...data, failureHandling: val } })
                  }
                >
                  <SelectTrigger className="h-6 w-[140px] text-[10px] px-2 py-0 nodrag">
                    <SelectValue />
                  ...
                  <SelectContent>
                    <SelectItem value="Drop" className="text-xs">
                      Drop
                    </SelectItem>
                    <SelectItem value="Retry" className="text-xs">
                      Retry
                    </SelectItem>
                    <SelectItem value="Retry + DLQ" className="text-xs">
                      Retry + DLQ
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {implementation === "RabbitMQ" && (
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(val) =>
                      updateNode(id, { data: { ...data, durable: val } })
                    }
                  />
                </div>
              )}
              ...
              {implementation === "RabbitMQ" && (
                <>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      className="h-6 text-[10px] w-28 bg-background nodrag"
                      placeholder="Exchange Name"
                      value={data.rabbitExchange || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, rabbitExchange: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      value={data.rabbitRoutingKey || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, rabbitRoutingKey: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      value={data.rabbitBindings || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, rabbitBindings: e.target.value },
                        })
                      }
                    />
                  </div>
                ...
                <>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      placeholder="e.g. 30s"
                      value={data.sqsVisibilityTimeout || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: {
                            ...data,
                            sqsVisibilityTimeout: e.target.value,
                          },
                        })
                      }
                    />
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, sqsDelay: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      id={`sqs-fifo-${id}`}
                      className="nodrag scale-75 origin-right"
                      checked={data.sqsFifo || false}
                      onCheckedChange={(val) =>
                        updateNode(id, { data: { ...data, sqsFifo: val } })
                      }
                    />
                  </div>
                </>
              ...
              {implementation === "Azure Service Bus" && (
                <>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      className="h-6 text-[10px] w-full bg-background nodrag"
                      placeholder="my-queue"
                      value={data.azureTopic || ""}
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, azureTopic: e.target.value },
                        })
                      }
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateNode(id, {
                          data: { ...data, azureSubscription: e.target.value },
                        })
                      }
                    />
                  </div>
