...
export const LangGraphCanvasMiddlewareNode = ({ id, data, selected }: NodeProps<MiddlewareNode>) => {
  ...
  const getTypeIcon = () => {
    switch (data.type) {
      case MIDDLEWARE_TYPE_HUMAN_IN_THE_LOOP: return <UserCheck className="w-3.5 h-3.5 text-purple-400" />;
      case MIDDLEWARE_TYPE_RATE_LIMIT: return <Gauge className="w-3.5 h-3.5 text-amber-400" />;
      case MIDDLEWARE_TYPE_LOGGING_TRACING: return <Activity className="w-3.5 h-3.5 text-sky-400" />;
      case MIDDLEWARE_TYPE_SUMMARIZATION: return <FileText className="w-3.5 h-3.5 text-emerald-400" />;
      case MIDDLEWARE_TYPE_MODEL_CALL_LIMIT: return <Cpu className="w-3.5 h-3.5 text-rose-400" />;
      case MIDDLEWARE_TYPE_TOOL_CALL_LIMIT: return <Wrench className="w-3.5 h-3.5 text-orange-400" />;
      case MIDDLEWARE_TYPE_MODEL_FALLBACK: return <GitFork className="w-3.5 h-3.5 text-blue-400" />;
      case MIDDLEWARE_TYPE_PII_DETECTION: return <Lock className="w-3.5 h-3.5 text-red-400" />;
      case MIDDLEWARE_TYPE_TODO_LIST: return <ListTodo className="w-3.5 h-3.5 text-teal-400" />;
      case MIDDLEWARE_TYPE_LLM_TOOL_SELECTOR: return <Filter className="w-3.5 h-3.5 text-indigo-400" />;
      case MIDDLEWARE_TYPE_TOOL_RETRY: return <RotateCcw className="w-3.5 h-3.5 text-yellow-400" />;
      case MIDDLEWARE_TYPE_MODEL_RETRY: return <RefreshCw className="w-3.5 h-3.5 text-amber-400" />;
      case MIDDLEWARE_TYPE_LLM_TOOL_EMULATOR: return <Terminal className="w-3.5 h-3.5 text-cyan-400" />;
      case MIDDLEWARE_TYPE_CONTEXT_EDITING: return <Scissors className="w-3.5 h-3.5 text-pink-400" />;
      case MIDDLEWARE_TYPE_PROVIDER_TOOL_SEARCH: return <Search className="w-3.5 h-3.5 text-violet-400" />;
      case MIDDLEWARE_TYPE_FILESYSTEM: return <FolderGit2 className="w-3.5 h-3.5 text-fuchsia-400" />;
      case MIDDLEWARE_TYPE_SUBAGENT: return <Users className="w-3.5 h-3.5 text-emerald-500" />;
  ...
  const getTypeLabel = () => {
    switch (data.type) {
      case MIDDLEWARE_TYPE_HUMAN_IN_THE_LOOP: return "Human in the Loop";
      case MIDDLEWARE_TYPE_RATE_LIMIT: return "Rate Limiter";
      case MIDDLEWARE_TYPE_LOGGING_TRACING: return "Logging & Tracing";
      case MIDDLEWARE_TYPE_SUMMARIZATION: return "Summarization";
      case MIDDLEWARE_TYPE_MODEL_CALL_LIMIT: return "Model Call Limit";
      case MIDDLEWARE_TYPE_TOOL_CALL_LIMIT: return "Tool Call Limit";
      case MIDDLEWARE_TYPE_MODEL_FALLBACK: return "Model Fallback";
      case MIDDLEWARE_TYPE_PII_DETECTION: return "PII Detection";
      case MIDDLEWARE_TYPE_TODO_LIST: return "To-do List";
      case MIDDLEWARE_TYPE_LLM_TOOL_SELECTOR: return "LLM Tool Selector";
      case MIDDLEWARE_TYPE_TOOL_RETRY: return "Tool Retry";
      case MIDDLEWARE_TYPE_MODEL_RETRY: return "Model Retry";
      case MIDDLEWARE_TYPE_LLM_TOOL_EMULATOR: return "LLM Tool Emulator";
      case MIDDLEWARE_TYPE_CONTEXT_EDITING: return "Context Editing";
      case MIDDLEWARE_TYPE_PROVIDER_TOOL_SEARCH: return "Provider Tool Search";
      case MIDDLEWARE_TYPE_FILESYSTEM: return "Filesystem Memory";
      case MIDDLEWARE_TYPE_SUBAGENT: return "Subagent Middleware";
      case MIDDLEWARE_TYPE_CUSTOM:
    ...
    >
      ...
      <div className="p-3 flex flex-col gap-2 text-xs">
        ...
        {data.type === "summarization" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            <span className="text-emerald-500 font-bold">Auto Summarization</span>
            <span className="text-[10px] text-muted-foreground">Model: {data.summarizationConfig?.model || "gpt-5.4-mini"}</span>
            <span className="text-[10px] text-muted-foreground">Keep: {data.summarizationConfig?.keepMessages ?? 20} msgs</span>
          </div>
        )}
        ...
        {data.type === "model_call_limit" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="font-bold">{data.modelCallLimitConfig?.runLimit ?? 5} / run</span>
          </div>
        )}

        {data.type === "tool_call_limit" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            <span>Tool Limit: {data.toolCallLimitConfig?.toolName || "All Tools"}</span>
            <span className="text-[10px] text-muted-foreground">Max {data.toolCallLimitConfig?.runLimit ?? 10} calls/run</span>
          </div>
        )}
        ...
        {data.type === "model_fallback" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground truncate">
              {data.modelFallbackConfig?.fallbackModels?.join(", ") || "gpt-5.4-mini, claude-3-5-sonnet"}
            </span>
          </div>
        )}
        ...
        {data.type === "pii_detection" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            <span className="capitalize">PII: {data.piiConfig?.piiType || "email"}</span>
        ...
        {data.type === "todo_list" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground">write_todos tool</span>
          </div>
        )}
        ...
        {data.type === "llm_tool_selector" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground">Max tools: {data.llmToolSelectorConfig?.maxTools ?? 3}</span>
          </div>
        )}
        ...
        {data.type === "tool_retry" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="font-bold text-yellow-500">{data.toolRetryConfig?.maxRetries ?? 3} retries</span>
          </div>
        )}
        ...
        {data.type === "model_retry" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="font-bold text-amber-500">{data.modelRetryConfig?.maxRetries ?? 3} retries</span>
          </div>
        )}
        ...
        {data.type === "context_editing" && (
          <div className="flex items-center justify-between text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground">Keep recent {data.contextEditingConfig?.keep ?? 3}</span>
          </div>
        )}
        ...
        {data.type === "filesystem" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            <span className="text-fuchsia-400 font-bold">Filesystem Memory</span>
            <span className="text-[10px] text-muted-foreground">Mode: {data.filesystemConfig?.backend || "composite"}</span>
          </div>
        )}
        ...
        {data.type === "subagent" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground">Task isolation</span>
          </div>
        )}
        ...
        {data.type === "custom" && (
          <div className="flex flex-col gap-1 text-[11px] text-foreground bg-secondary/20 p-2 rounded border border-border/50 font-mono">
            ...
            <span className="text-[10px] text-muted-foreground">User logic</span>
          </div>
        )}
      </div>
    </div>
